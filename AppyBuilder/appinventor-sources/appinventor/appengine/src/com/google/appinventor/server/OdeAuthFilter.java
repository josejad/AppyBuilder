...
public class OdeAuthFilter implements Filter {
  ...
  long lastTimeWhitelistLoaded = new Date().getTime();
...
//    }
  ...
  boolean isUserWhitelisted() {
    // For each USER requests, we basically re-load the whitelist every 60 minutes.
    // This way, the whitelist IS NOT reloaded on every user-request
    long now = new Date().getTime();
    long diffInMillis = now - lastTimeWhitelistLoaded;
    long diffHours = diffInMillis / (60 * 60 * 1000) % 24;
    double diffMinutes = diffInMillis / (60 * 1000) % 60;

    if (diffMinutes > 30) {
      whitelist = new Whitelist();
      lastTimeWhitelistLoaded = new Date().getTime();
    }
...
//    return storageIo.checkWhiteList(localUser.getUserEmail());
  ...
  void writeWhitelistErrorMessage(HttpServletResponse response) throws IOException {
    ...
    out.print("You are attempting to connect to this AppyBuilder service with the login ID:\n\n" +
