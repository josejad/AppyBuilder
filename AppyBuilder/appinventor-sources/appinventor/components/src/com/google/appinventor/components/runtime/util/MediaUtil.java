...
//        return foo(mediaPath);
  ...
  private static InputStream foo(final String mediaPath) {
    final InputStream[] inputStream = {null};
    Thread thread = new Thread(new Runnable() {
      @Override
      public void run() {
        try {
          inputStream[0] = new URL(mediaPath).openStream();
        } catch (Exception e) {
          Log.e(LOG_TAG, "Error. Unable to load media from url: " + e.getMessage());
        } finally {
